<template>
  ...
  >
    <template v-slot:append>
      ...
      > <q-tooltip>
          Adjust filter to display/hide templates..
        </q-tooltip>
      </q-btn>
    </template>
    <!-- 
    ...
    <q-dialog v-model="saveTemplateDialog">
      <q-card class="q-pa-md" style="min-width: 500px">
        ...
        <q-card-actions align="right">
          <q-btn flat label="Ovewrite selected" color="primary" v-close-popup @click="saveUserSettings()"/>
          <q-btn flat label="Add new template" color="primary" v-close-popup @click="saveUserSettings()"/>
        </q-card-actions>
      </q-card>
    </q-dialog>    
    <q-dialog v-model="configureOrdertypesDropdown">
      <q-card>
        ...
        <q-card-actions align="right">
          <q-btn flat label="Save" color="primary" v-close-popup @click="saveUserSettings()"/>
        </q-card-actions>
      </q-card>
    </q-dialog>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const fileTemplateLabel = (fileTemplate: FileTemplate | undefined): string => {
    ...
    const allFileTemplateTagsArray = computed(() => { 
      const nonUniqueTags = fileTemplates.value?.flatMap(t => t.templateTags.split(','))
      return [...new Set(nonUniqueTags)];
    })
    const filteredFileTemplateTags = ref<string[]>();
    const filterFileTemplateTags = (val:string, update:(fn:()=>void)=>void) => {
        update(() => {
          if (val === '') {
            ...
            filteredFileTemplateTags.value = allFileTemplateTagsArray.value?.filter(
              v => v.toLowerCase().indexOf(needle) > -1
            )
          }
        })
      }
    const createFileTemplateTag = (val:string, done:(val:string, res:string)=>void):void => {
    ...
    return { fileTemplateVal, configureOrdertypesDropdown, saveUserSettings, fileTemplates, fileTemplateLabel, saveTemplateDialog, 
      saveAsNew, selectedFileTemplateTags, filteredFileTemplateTags, filterFileTemplateTags, createFileTemplateTag};
  },
});
</script>

...
class FileUpload(
    ...
    fun sendFile(
        ...
        logger.info("Start uploading file via EBICS sessionId=${ebicsSession.sessionId}, userId=${ebicsSession.user.userId}, partnerId=${ebicsSession.user.partner.partnerId}, bankURL=${ebicsSession.user.partner.bank.bankURL}, order=$uploadOrder, file length=${content.size}")
        ...
        while (state.hasNext()) {
            ...
            sendFileSegment(
                ...
                state.transactionId, uploadOrder.orderType ?: "FUL", traceSession, httpSession
            )
        }
        logger.info("Finished uploading file via EBICS sessionId=${ebicsSession.sessionId}, userId=${ebicsSession.user.userId}, partnerId=${ebicsSession.user.partner.partnerId}, bankURL=${ebicsSession.user.partner.bank.bankURL}, order=$uploadOrder, file length=${content.size}, orderNumber=${response.orderNumber}, transactionId=${response.transactionId.toHexString()}")
